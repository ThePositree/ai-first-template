...
- Renamed root `PROJECT.md` to `PRODUCT.md`, renamed the installable envelope context file from `.ai-first/context/PROJECT.md` to `.ai-first/context/PRODUCT.md`, clarified that source project Markdown does not mean a root `.ai-first` workspace, and removed the source-repository proof constraint from the progressive memory graph Wiki migration path.
...
- Added the full `.ai-first/context` memory set to the source repository and updated `doctor.js` to verify the required memory files.
- Completed the Local Memory Schema task and moved active work to fresh-chat context recovery.
- Separated the installer from installed maintenance scripts: local builds now emit `dist/install.js`, while installed `.ai-first/scripts` contains `doctor.js` and `repair.js`.
- Added the local installer script and verified installation plus `doctor.js` in a temporary target project.
- Refined task memory planning by merging conflict reporting into doctor output, removing the existing-repository analyzer idea, and adding backlog/ideas stewardship as approved work.
- Reshaped the installable envelope so root `AGENTS.md` is a thin handoff to `.ai-first/README.md` and durable project memory lives under `.ai-first/context`.
- Reframed `ai-first` as a small repository memory and maintenance workspace that reduces repeated owner explanations and helps agents recover context across chats.
- Replaced the modular template concept with a single installable envelope to keep `ai-first` focused on agent productivity rather than reusable templates.
- Added a task-delta communication rule so agents summarize changes to active work, backlog, and ideas instead of silently mutating project state.
- Removed roadmap files from the source project and installable envelope in favor of active work, approved backlog, saved ideas, and changelog history.
- Verified `npm install`, `npm run build`, and `node .ai-first/scripts/doctor.js` after npm registry access became available.
