...
If the owner asks to move `.ai-first` task state into GitHub or another tracker, treat it as a migration between task sources of truth, not background mirroring. Run a fresh briefing first unless the owner explicitly says to reuse an existing recorded policy. Record the approved policy in `.ai-first/context/github-sync.md` when GitHub is involved, but do not treat that file as permission for future external writes. The owner-facing workflow is a natural-language request, not a required CLI, skill, or prepared script. Issues are the portable GitHub baseline; Projects, Wiki, and Discussions are optional when selected. If a selected surface is disabled or inaccessible, ask whether the owner wants it enabled, explain any owner-side setup needed, and wait or fall back only after approval. Show the planned external entities and get explicit apply confirmation before writing. Keep tracker-only relationship links out of normal `.ai-first` memory files unless the owner explicitly approves that tracker as a source of truth.
